...
class Model {
  ...
  constructor (server, settings, values) {
    ...
    // @todo: move everything bellow to a single hidden property 'metadata'
    // to reduce reserved words.
  ...
  hydrate(values, isNew) {
    ...
    for (let fieldName in fields) {
      if (fieldName in values) {
        let fieldSettings = fields[fieldName];
        if (fieldSettings.type == 'reference') {
          this[fieldName] = this.normalizeReferenceFieldValue(fieldSettings, values[fieldName])
        }
        else {
  ...
  /**
   * Normalize reference field value. Make the value an instance or instances
   * of the referenced type.
   *
   * @param {Object} fieldSettings Field settings object.
   * @param {Mixed} value The value for the field, either an object or an array
   *   of objects.
   */
  normalizeReferenceFieldValue(fieldSettings, value) {
    let referenceModel = this.server.model(fieldSettings.reference);
    if (fieldSettings.multiple) {
      return value.map((item) => {
        return item instanceof referenceModel ? item : new referenceModel(item);
      });
    }
    else {
      return value instanceof referenceModel ? value : new referenceModel(value);
    }
  }
